...
I have been debugging an issue with Cloudflare One today - CF's official zero-touch client for Android - not receiving managed configurations through MDM.
...
..it also doesn't align with [their documentation](https://developers.cloudflare.com/cloudflare-one/team-and-resources/devices/warp/deployment/mdm-deployment/#android--chromeos).

The documentation shows the app expects this flat configuration: 
...
That unfortunately is not what they publish, and so won't be how an MDM would import the configuration for users to set.

To clear any further doubt between the app and documentation, I pulled their `app_restrictions.xml` file from an APK directly, showing:
...
With how they've defined their application restrictions XML clearly built with a `android:restrictionType="bundle_array"`, an MDM won't send out a flat configuration the documentation (and application) demands without manual modification, so when sending the config from MDM, the device is receiving (as confirmed via [Package Search](/projects/package-search)'s managed config tool):
...
This seems like something CloudFlare needs to fix in an updated version. I should imagine MDMs like Intune can get around this by allowing direct modification of the JSON being sent out, but for those platforms that simply consume the managed properties and turn them into a form.. that's troublesome.
...
2. Adjust the application restrictions to flatten down the configuration such that it matches the documentation. This will require users who have configured with managed properties to date to submit a new configuration.

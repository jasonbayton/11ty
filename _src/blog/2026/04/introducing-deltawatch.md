...
Existing tools occupy one of two camps. Commercial services like Visualping, Distill, and Wachete are polished but genuinely expensive at any usable scale - £10 to £15 a month for monitoring a few dozen pages with reasonable check frequency is the going rate, and their free tiers are either time-limited or limited to a handful of watches with hour-plus check intervals. The open-source alternative - changedetection.io - is fantastic, but it's a self-hosted service, which means it's *my* problem to keep running, update, back up, and debug at 2am when the Docker container decides it's had enough.

What I wanted was a hosted service that priced sensibly, had no time limit on the free tier, gave me the surgical content-extraction controls I need for the long-tail pages I actually care about, and didn't require me to become an SRE for my own monitoring stack.
...
Content extraction gets you surgical targeting. The **conditions engine** filters out the noise that slips through anyway. Eight operators (contains, does not contain, equals, greater than, regex match, and friends), AND/OR logic, and change-percentage thresholds so you only get alerted when the page changes by more than, say, 5%. Every condition composes with every other condition. It's the difference between getting one useful alert a week and getting twelve useless alerts a day.

### Notifications where you already work

Email is built in. Everything else is a URL away: **Slack, Discord, Telegram, Microsoft Teams, and generic webhooks**. Add channels per watch, per tag, or at the workspace level. Every channel is available on every plan, including the free tier, because pay-walling notifications on a monitoring product is absurd.
...
From the graph (or a table view, if you prefer rows), you can **promote any discovered page into a real watch** with one click - or bulk-promote a dozen at a time. Promotion routes through the standard watch creation flow, so plan limits, duplicate detection, and interval enforcement all still apply. There's also an optional auto-watch toggle that turns every newly discovered page into a watch as the crawler finds it, which is excellent for small focused sites and *absolutely terrifying* for anything the size of a Google docs property.

Sites also does **recrawl scheduling**. When a watch detects a change, the parent site queues an automatic re-crawl (debounced at 12 hours site-wide to avoid storms), so the data stays current with the pages you care about. As a fallback, every site gets a scheduled full recrawl roughly every 7 days with ±40% per-site jitter, so abandoned sites still stay fresh and the fleet doesn't all crawl at once.
...
Workspaces are per-team, per-project, or per-client containers. Each workspace has its own plan, its own billing, its own users, and its own notification defaults. I run a personal workspace for my own monitoring, a bayton.org workspace for the documentation side, and a handful of client workspaces that are billed separately. You only pay for the workspaces that need paid features.

Every workspace can generate a full ZIP **backup** in the background - watches, tags, settings, and snapshot history - and download it via a time-limited link. No lock-in: if you ever want to leave, the exit door is right there.

## Under the hood

For those of you who like a peek behind the curtain, here's the architecture.

**Frontend** is React 19 with Vite and Tailwind CSS 4. The Watchlist, site graph, diff viewer, and admin panels are all SPA components. The site graph uses a force-directed canvas simulation with viewport culling, zoom-based label visibility, and adaptive force parameters so it stays responsive at 1,000+ nodes. No D3 in the main bundle; it's a lightweight hand-rolled force simulation that weighs less than most charting libraries.

**Backend** is Express 5 with a comprehensive REST API. It runs in two deployment modes: as a Netlify full-stack application (SPA + serverless functions) lighter instances, and as a standalone Node.js process with a strongly-consistent PostgreSQL backend. The standalone build supports a split deployment topology where HTTP serving and background work run as separate processes, isolating API latency from crawl and watch-check load.

**Storage** is a pluggable backend abstraction. There are four backends - Netlify Blobs, Directus, a generic KVP-over-JSONB database store, and a native-columns Postgres store that uses typed columns and expression indexes instead of JSONB blobs for the hot tables. The native backend is what runs on the VPS in production, and it's materially faster than the KVP equivalent for listing, filtering, and bulk operations.
...
**Platform health monitoring** runs every 5 minutes on the worker process and captures disk usage, Node memory, database pool pressure, watch-check failure rate, crawl job health, scheduler miss detection, and API p95 latency. Alerts only fire on state transitions (`ok → warn → critical`) with a configurable cooldown, and there's a recovery notification when everything returns to `ok`. It's the kind of operational visibility you'd expect from a commercial SaaS, because that's what this is.

## Pricing

I wanted the pricing to be obvious, cheap, and sustainable. Three tiers:

- **Ping** - **free, forever, no time limit.** 5 watches, 3-hour minimum interval, 10 visible snapshots, all notification channels. Enough to monitor the pages you actually care about without paying a penny. No credit card required to sign up.
- **Pulse** - **£3/month.** 50 watches, 1-hour minimum interval, 100 visible snapshots, API access.
...
- **Not self-hosted.** DeltaWatch is a hosted SaaS. I built it as a product, not a self-host kit, and the operational story is dramatically better when there's exactly one version of the platform in the wild. If you want self-hosted, changedetection.io is an excellent choice.
